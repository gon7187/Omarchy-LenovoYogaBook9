...
def check():
    ...
    assert fan.Curve(fan.DEFAULT_STEPS).targets(101, 0) == (56, 94)
    curve = fan.Curve(fan.DEFAULT_STEPS)
    assert curve.targets(69.9, 0) == (0, 0)
    assert curve.targets(80, 1) == (51, 82)
    assert curve.targets(100, 2) == (56, 94)
    assert curve.targets(99, 40) == (56, 94)
    assert curve.targets(97, 41) == (56, 94)
    assert curve.targets(96, 42) == (56, 94)
    assert curve.targets(98, 60) == (56, 94)  # Rewarming cancels the delay.
    assert curve.targets(80, 61) == (56, 94)
