...
- Компактная верхняя строка: Esc, F1–F12, Ins, PrtSc, слова, настройки и закрытие. Fn расположен рядом с Ctrl; пробел без надписи. Цифровой ряд на 30% ниже буквенного; четыре полноразмерные стрелки образуют перевёрнутую Т. Язык переключается Alt+Shift или кнопкой RU/EN в нижнем ряду.
...
- Тумблер **Настройки → Ввод → Тема OLED**: чисто чёрный фон и только контуры
  клавиш. На OLED чёрный пиксель не горит, поэтому открытая клавиатура
  почти не тратит энергию. Цвета обеих тем — в `Theme.qml`.
