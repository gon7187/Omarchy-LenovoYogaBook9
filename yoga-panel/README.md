...
- Компактная верхняя строка: Esc, F1–F12, Ins, PrtSc, настройки и закрытие. Fn расположен рядом с Ctrl; пробел без надписи. Цифровой ряд на 30% ниже буквенного; четыре полноразмерные стрелки образуют перевёрнутую Т. Язык переключается Alt+Shift или кнопкой RU/EN в нижнем ряду.
...
- **Слова** показывает или скрывает три офлайн-подсказки по префиксу от двух букв.
  Подсказки появляются отдельной строкой под клавиатурой, занимая часть высоты
  тачпада; сами клавиши не сдвигаются. Надпись «Готово» скрыта.
...
- **Fn** можно нажать перед одной клавишей или удерживать. **Fn+Esc** фиксирует
  дополнительные действия; повторное Fn+Esc снимает фиксацию. При FnLock удержание
  Fn временно возвращает F1–F12. Esc без Fn всегда остаётся Escape. При закрытии
  панели состояние Fn сбрасывается. Маленькие значки и рамки подсвечивают действия.
  F1–F12 без Fn передаются приложению, включая сочетания с Ctrl/Alt/Shift/Super.

| Сочетание | Действие |
|---|---|
| Fn + F1 / F2 / F3 | Выключить звук / тише / громче |
| Fn + F4 | Выключить или включить микрофон |
| Fn + F5 / F6 | Яркость обоих экранов − / + |
| Fn + F7 / F8 / F9 | Меню экранов / меню Omarchy / блокировка сеанса |
| Fn + F10 / F11 / F12 | Тачпад вкл./выкл. / подсказки / настройки панели |
| Fn + ← / → / ↑ / ↓ | Home / End / Page Up / Page Down |
| Fn + Backspace / \ | Выбор профиля питания / настройки звука |
| Fn + Enter / правый Shift / пробел | Ночной свет / обзор окон / OLED-тема |
| PrtSc | Выбор области снимка экрана |

Системные действия используют установленный Omarchy; это Linux-аналоги фирменных
функций Lenovo. Пока команда громкости выполняется, новые команды громкости
пропускаются, чтобы параллельное чтение/изменение уровня не теряло результат.
Недоступная команда показывает ошибку на панели и не блокирует набор текста.
...
- Двумя пальцами вбок (вбок вдвое больше, чем по вертикали) — переключить окно, как
  прокрутка: пальцы влево — окно справа (`Super+→`), вправо — окно слева; каждые
  150 px — ещё одно окно. Горизонтальной прокрутки в приложениях с тачпада нет.
...
- Кнопка настроек (значок справа вверху): скорость, ускорение, прокрутка. Сохраняются в
  `~/.config/yoga-panel/settings.json`. 0 ускорения означает постоянную скорость.
- **Настройки → Внешний вид**: тема «Как в ОС» автоматически следует текущей
  Omarchy; можно выбрать любую установленную палитру вручную. Системные файлы
  `/usr/share/omarchy/themes/*/colors.toml` дополняются пользовательскими
  `~/.config/omarchy/themes/*/colors.toml`. Текущая тема читается из
  `~/.local/state/omarchy/current/`; проверка изменений раз в 2 секунды.
  Повреждённый current сохраняет последнюю рабочую палитру, удалённая ручная
  тема временно заменяется текущей системной. Старый `oledTheme` оставлен для
  совместимости настроек; внешний вид теперь задаёт `themeName`.
- Там же: непрозрачность заливок **65–100%** и стиль значков
  **По теме / Пиксельные / Контурные / mic**. Сердце — прежняя клавиша Super.
  Fn+пробел переключает **OLED Black / Как в ОС**. На чёрной теме 100% оставляет
  чёрные пиксели выключенными, прозрачность показывает находящиеся под панелью обои.
- **OLED**: сдвиг максимум ±2 физических пикселя каждые 3 минуты после паузы
  ввода 3 секунды; при касании движение прекращается. Через минуту без ввода
  панель приглушается; первое касание сразу работает и возвращает яркость.
  Настройки и диктовка приостанавливают эффекты. Оба эффекта можно отключить.
  Это снижение риска выгорания, без гарантии защиты. Старые настройки тачпада
  при обновлении сохраняются. В планшетном режиме настройки заменяют клавиатуру.
...
node test_fn_keys.cjs
node test_appearance.cjs
python3 -m unittest test_themes test_defaults_and_language
QT_QPA_PLATFORM=offscreen QT_QUICK_BACKEND=software /usr/lib/qt6/bin/qmltestrunner -input tst_oled_care.qml
QT_QPA_PLATFORM=offscreen QT_QUICK_BACKEND=software /usr/lib/qt6/bin/qmltestrunner -input tst_theme.qml
...
Свайпы переключают только верхний `eDP-1`: влево — 1 → 3 → 4 → … → 10 → 1,
вправо — в обратном порядке. Workspace 2 зарезервирован для нижнего экрана.
