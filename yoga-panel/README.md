...
- Компактная верхняя строка: слова, настройки, Esc и закрытие. Язык переключается Alt+Shift или кнопкой RU/EN в нижнем ряду.
...
- **Alt+Shift** в любом порядке на экранных клавишах меняет язык и в системе.
  ...
  Кнопка RU/EN срабатывает один раз по завершении касания. Повторные системные
...
и службы, настройки яркости, звука, диктовки и меню. Они остаются только на
...
После быстрого движения двумя пальцами добавляется короткая затухающая инерция
(до 300 мс). Пауза перед отпусканием исключает докручивание. Новое касание
тачпада, ввод клавиши, смена окна, закрытие панели или изменение скорости
останавливают инерцию. Переключатель **Инерция прокрутки** в настройках
сохраняется отдельно. Ползунок скорости применяется во время перетаскивания;
кнопки ± меняют коэффициент на 0,01 вместо 0,03. 100% соответствует исходному
коэффициенту 0,18; старые значения сохраняются без перенастройки.

Тесты `test_touchpad.cjs` проверяют затухание без обратного скачка,
остановку при касании/отмене, отсутствие инерции после паузы и моментальное
применение нового коэффициента. `tests/tst_scroll_setting.qml` проверяет
реальное перемещение ползунка и сохранение привязки значения в Qt.
