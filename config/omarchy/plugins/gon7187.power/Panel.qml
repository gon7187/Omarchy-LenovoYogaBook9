...
Panel {
  ...
  readonly property string chargeLimitShell: "dev=$(upower -e | grep -m1 BAT) || exit 1; "
  ...
  readonly property string heroStatusText: {
    if (fullyCharged) return "Полностью заряжена"
  ...
  function updateChargeLimit(raw) {
    ...
    chargeLimitSupported = lines[0].trim() === "b true"
    chargeLimitEnabled = lines[1].trim() === "b true"
  }

  function setChargeLimit(enabled) {
    if (chargeLimitSetProc.running) return
    chargeLimitEnabled = enabled
    chargeLimitSetProc.command = ["sh", "-c", chargeLimitShell +
      "busctl call org.freedesktop.UPower \"$dev\" org.freedesktop.UPower.Device EnableChargeThreshold b " + (enabled ? "true" : "false")]
  ...
  Process {
    id: chargeLimitProc
    command: ["sh", "-c", root.chargeLimitShell +
      "busctl get-property org.freedesktop.UPower \"$dev\" org.freedesktop.UPower.Device ChargeThresholdSupported ChargeThresholdEnabled"]
      ...
      Column {
        ...
        Row {
          ...
          Column {
            ...
            InfoPair { label: "Ёмкость батареи"; value: root.batteryInfo.size || "" }
            InfoPair { label: "Циклы зарядки"; value: root.batteryInfo.cycles || "—" }
          }
          ...
          Column {
            ...
            spacing: Style.spacing.labelGap
            InfoPair {
              label: root.chargeThresholdActive ? "Лимит зарядки" : (root.discharging ? "Осталось времени" : "До полной зарядки")
              value: root.chargeThresholdActive ? (root.batteryInfo.threshold || (root.chargeLimitEnabled ? "Вкл." : "-")) : (root.batteryFlowIdle ? "-" : (root.batteryInfo.time || "—"))
            }
            InfoPair {
              label: root.chargeThresholdActive ? "Состояние батареи" : (root.discharging ? "Разряжается" : "Заряжается")
              value: root.chargeThresholdActive ? "Ожидание" : (root.batteryFull ? "-" : (root.batteryInfo.rate || ""))
            }
          }
        }
        ...
        Toggle {
          ...
          width: parent.width
          label: "Лимит зарядки"
          description: "Останавливать зарядку раньше, чтобы продлить срок службы батареи"
          foreground: root.bar.foreground
