...
Panel {
  ...
  function toggleAutoBrightness() {
    if (autoBrightnessToggleProc.running) return
    root.autoBrightnessBusy = true
    autoBrightnessToggleProc.running = true
  }
  ...
  // Auto-brightness is a user service, separate from the display backlight
  // state reported by omarchy-monitor-state.
  Process {
    id: autoBrightnessStateProc
    command: ["systemctl", "--user", "is-active", "--quiet", "yoga-autobrightness.service"]
    onExited: function(exitCode) { root.autoBrightnessEnabled = exitCode === 0 }
  }

  Process {
    id: autoBrightnessToggleProc
    command: ["/home/gon7187/.local/bin/yoga-autobrightness-toggle"]
    stdout: StdioCollector { waitForEnd: true }
    onExited: function(exitCode) {
      root.autoBrightnessBusy = false
      root.refresh()
    }
  }
    ...
    PanelKeyCatcher {
      ...
      ScrollView {
        ...
        Column {
          ...
          Item {
            width: parent.width
            implicitHeight: Math.max(heroIcon.implicitHeight, heroLabels.implicitHeight, autoBrightnessSwitch.implicitHeight)
            ...
            Column {
              ...
              anchors.rightMargin: autoBrightnessSwitch.width + Style.space(10)
              anchors.verticalCenter: parent.verticalCenter
            ...
            // Auto-brightness lives with the monitor controls instead of the
            // launcher menu. Its state comes directly from the user service.
            ToggleSwitch {
              id: autoBrightnessSwitch
              checked: root.autoBrightnessEnabled
              busy: root.autoBrightnessBusy
