...
Item {
  ...
  function startPhase(nextPhase) {
    ...
    phase = nextPhase
    stderrText = ""
    // Retries the fast.com endpoint request, which the shared VPN exit gets rate limited on.
    speedTestProc.command = ["sh", "-c", "exec \"$HOME/.local/bin/yoga-network-speedtest\" \"$1\"", "sh", nextPhase]
